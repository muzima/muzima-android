   ...
   document.setupAutoCompleteWithOnchange = function (elementName, dataDictionary) {
    ...
    function displayObs(historyByData){
          ...
          $.each(historyByData, function (i,ob) {
                  observationdate = ob.observationDatetime;
                  levelone = JSON.stringify(ob.valueCoded);
                  levelone = JSON.parse(levelone);
                  levelone = JSON.stringify(levelone.conceptNames);
                  levelone = JSON.parse(levelone);
                   $.each(levelone, function (key,value) {
                        if(value.preferred){
                              j=j+1;
                              var row = table.insertRow(j);
                              var cell = row.insertCell(0);
                              cell.appendChild(document.createTextNode(observationdate));
                              var cell = row.insertCell(1);
