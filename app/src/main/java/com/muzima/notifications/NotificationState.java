...
public class NotificationState {
    ...
    public @Nullable
    Uri getRingtone(@NonNull Context context) {
        if (!notifications.isEmpty()) {
            SignalRecipient recipient = notifications.getFirst().getRecipient();
    ...
    public PendingIntent getRemoteReplyIntent(Context context, SignalRecipient recipient) {
    ...
    public PendingIntent getAndroidAutoReplyIntent(Context context, SignalRecipient recipient) {
        if (threads.size() != 1) throw new AssertionError("We only support replies to single thread notifications!");
