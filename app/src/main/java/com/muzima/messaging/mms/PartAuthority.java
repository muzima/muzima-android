...
import com.muzima.messaging.provider.PartProvider;
...
public class PartAuthority {
    private static final String PART_URI_STRING = "content://com.muzima/part";
    private static final String THUMB_URI_STRING = "content://com.muzima/thumb";
    ...
    static {
        ...
        uriMatcher.addURI("com.muzima", "part/*/#", PART_ROW);
        uriMatcher.addURI("com.muzima", "thumb/*/#", THUMB_ROW);
    ...
    public static InputStream getAttachmentStream(@NonNull Context context, @NonNull Uri uri)
        ...
        try {
            switch (match) {
                ...
                case PERSISTENT_ROW:
                    return PersistentBlobProvider.getInstance(context).getStream(context, ContentUris.parseId(uri));
                case SINGLE_USE_ROW:
                    return MemoryBlobProvider.getInstance().getStream(ContentUris.parseId(uri));
                default:
    ...
    String getAttachmentFileName(@NonNull Context context, @NonNull Uri uri) {
        int match = uriMatcher.match(uri);
    ...
    public static @Nullable
    Long getAttachmentSize(@NonNull Context context, @NonNull Uri uri) {
        int match = uriMatcher.match(uri);
    ...
    public static @Nullable
    String getAttachmentContentType(@NonNull Context context, @NonNull Uri uri) {
        int match = uriMatcher.match(uri);
