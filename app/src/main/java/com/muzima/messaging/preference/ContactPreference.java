...
public class ContactPreference extends Preference {
    ...
    public void setSecure(boolean secure) {
        ...
        if (secureCallButton != null)
            secureCallButton.setVisibility(secure ? View.VISIBLE : View.GONE);
        if (callButton != null) callButton.setVisibility(secure ? View.GONE : View.VISIBLE);

        int color;

        if (secure) {
            color = getContext().getResources().getColor(R.color.primary_color);
        } else {
        ...
        if (secureCallButton != null)
            secureCallButton.setColorFilter(color, PorterDuff.Mode.SRC_IN);
    ...
    public void setListener(Listener listener) {
        ...
        if (this.messageButton != null)
            this.messageButton.setOnClickListener(v -> listener.onMessageClicked());
        if (this.secureCallButton != null)
            this.secureCallButton.setOnClickListener(v -> listener.onSecureCallClicked());
        if (this.callButton != null)
            this.callButton.setOnClickListener(v -> listener.onInSecureCallClicked());
    }
