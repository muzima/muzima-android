...
import com.muzima.messaging.components.QuoteView;
...
import com.muzima.messaging.utils.LongClickMovementMethod;
...
public class ConversationItem extends LinearLayout
    ...
    private @NonNull SignalRecipient conversationRecipient;
    ...
    private void setAudioViewTint(MessageRecord messageRecord, SignalRecipient recipient) {
        if (messageRecord.isOutgoing()) {
    ...
    private void setContactPhoto(@NonNull SignalRecipient recipient) {
    ...
    private void setQuote(@NonNull MessageRecord current, @NonNull Optional<MessageRecord> previous, @NonNull Optional<MessageRecord> next, boolean isGroupThread) {
        if (current.isMms() && !current.isMmsNotification() && ((MediaMmsMessageRecord) current).getQuote() != null) {
            ...
            assert quote != null;
            quoteView.setQuote(glideRequests, quote.getId(), SignalRecipient.from(context, quote.getAuthor(), true), quote.getText(), quote.isOriginalMissing(), quote.getAttachment());
    ...
    private void setGroupMessageStatus(MessageRecord messageRecord, SignalRecipient recipient) {
    ...
    @Override
    public void onModified(final SignalRecipient modified) {
        Util.runOnMain(() -> {
            setBubbleState(messageRecord);
            setContactPhoto(recipient);
            setGroupMessageStatus(messageRecord, recipient);
    ...
    private class SharedContactEventListener implements SharedContactView.EventListener {
        ...
        @Override
        public void onInviteClicked(@NonNull List<SignalRecipient> choices) {
            if (eventListener != null && batchSelected.isEmpty()) {
        ...
        @Override
        public void onMessageClicked(@NonNull List<SignalRecipient> choices) {
            if (eventListener != null && batchSelected.isEmpty()) {
...
//                context.startActivity(intent);
    ...
    private void handleMessageApproval() {
        ...
        builder.setPositiveButton(R.string.general_yes, (dialogInterface, i) -> {
            if (messageRecord.isMms()) {
                MmsDatabase database = DatabaseFactory.getMmsDatabase(context);
                database.markAsInsecure(messageRecord.getId());
                database.markAsOutbox(messageRecord.getId());
                database.markAsForcedSms(messageRecord.getId());
//                TODO +++++++++++work on MmsSendJob
//                MuzimaApplication.getInstance(context)
//                        .getJobManager()
...
//                                messageRecord.getIndividualRecipient().getAddress().serialize()));
        ...
        builder.setNegativeButton(R.string.general_no, (dialogInterface, i) -> {
            if (messageRecord.isMms()) {
