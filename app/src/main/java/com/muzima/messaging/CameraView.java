...
import com.muzima.R;
import com.muzima.messaging.camera.CameraSurfaceView;
import com.muzima.messaging.camera.CameraUtils;
    ...
    private int getCameraPictureOrientation() {
        if (getActivity().getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED) {
            outputOrientation = getCameraPictureRotation(getActivity().getWindowManager()
                    .getDefaultDisplay()
                    .getOrientation());
        } else if (getCameraInfo().facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
