...
import com.muzima.messaging.sms.IncomingIdentityDefaultMessage;
import com.muzima.messaging.sms.IncomingIdentityUpdateMessage;
import com.muzima.messaging.sms.IncomingIdentityVerifiedMessage;
import com.muzima.messaging.sms.IncomingTextMessage;
import com.muzima.messaging.sms.OutgoingIdentityDefaultMessage;
import com.muzima.messaging.sms.OutgoingIdentityVerifiedMessage;
...
public class IdentityUtil {
    ...
    @UiThread
    public static ListenableFuture<Optional<IdentityRecord>> getRemoteIdentityKey(final Context context, final SignalRecipient recipient) {
        final SettableFuture<Optional<IdentityRecord>> future = new SettableFuture<>();

        new AsyncTask<SignalRecipient, Void, Optional<IdentityRecord>>() {
            @Override
            protected Optional<IdentityRecord> doInBackground(SignalRecipient... recipient) {
    ...
    public static void markIdentityVerified(Context context, SignalRecipient recipient, boolean verified, boolean remote) {
        ...
        while ((groupRecord = reader.getNext()) != null) {
            if (groupRecord.getMembers().contains(recipient.getAddress()) && groupRecord.isActive() && !groupRecord.isMms()) {
                ...
                if (remote) {
                    ...
                    SignalRecipient groupRecipient = SignalRecipient.from(context, SignalAddress.fromSerialized(GroupUtil.getEncodedId(group.getGroupId(), false)), true);
    ...
    public static void markIdentityUpdate(Context context, SignalRecipient recipient) {
        long time = System.currentTimeMillis();
        SmsDatabase smsDatabase = DatabaseFactory.getSmsDatabase(context);
        GroupDatabase groupDatabase = DatabaseFactory.getGroupDatabase(context);
        GroupDatabase.Reader reader = groupDatabase.getGroups();
    ...
    public static void processVerifiedMessage(Context context, VerifiedMessage verifiedMessage) {
        synchronized (SESSION_LOCK) {
            ...
            SignalRecipient recipient = SignalRecipient.from(context, SignalAddress.fromExternal(context, verifiedMessage.getDestination()), true);
    ...
    String getPluralizedIdentityDescription(@NonNull Context context,
                                            @NonNull List<SignalRecipient> recipients,
