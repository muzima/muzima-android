...
public class SmsDatabase extends MessagingDatabase {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                if (Types.isOutgoingMessageType(cursor.getLong(cursor.getColumnIndexOrThrow(TYPE)))) {
                    SignalAddress theirAddress = messageId.getAddress();
                    SignalAddress ourAddress = SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(ADDRESS)));
    ...
    public @NonNull
    Pair<Long, Long> insertReceivedCall(@NonNull SignalAddress address) {
        return insertCallLog(address, Types.INCOMING_CALL_TYPE, false);
    }

    public @NonNull
    Pair<Long, Long> insertOutgoingCall(@NonNull SignalAddress address) {
        return insertCallLog(address, Types.OUTGOING_CALL_TYPE, false);
    }

    public @NonNull
    Pair<Long, Long> insertMissedCall(@NonNull SignalAddress address) {
        return insertCallLog(address, Types.MISSED_CALL_TYPE, true);
    }

    private @NonNull
    Pair<Long, Long> insertCallLog(@NonNull SignalAddress address, long type, boolean unread) {
        SignalRecipient recipient = SignalRecipient.from(context, address, true);
    ...
    protected Optional<InsertResult> insertMessageInbox(IncomingTextMessage message, long type) {
        ...
        SignalRecipient recipient = SignalRecipient.from(context, message.getSender(), true);

        SignalRecipient groupRecipient;

        if (message.getGroupId() == null) {
            groupRecipient = null;
        } else {
            groupRecipient = SignalRecipient.from(context, message.getGroupId(), true);
        }
    ...
    public long insertMessageOutbox(long threadId, OutgoingTextMessage message,
        ...
        SignalAddress address = message.getRecipient().getAddress();
        Map<SignalAddress, Long> earlyDeliveryReceipts = earlyDeliveryReceiptCache.remove(date);
        Map<SignalAddress, Long> earlyReadReceipts = earlyReadReceiptCache.remove(date);
    ...
    public class Reader {
        ...
        public SmsMessageRecord getCurrent() {
            ...
            SignalAddress address = SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(SmsDatabase.ADDRESS)));
