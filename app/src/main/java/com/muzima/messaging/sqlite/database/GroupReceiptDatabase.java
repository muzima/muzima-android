...
public class GroupReceiptDatabase extends Database {
    ...
    public void insert(List<SignalAddress> addresses, long mmsId, int status, long timestamp) {
    ...
    List<GroupReceiptInfo> getGroupReceiptInfo(long mmsId) {
        ...
        try (Cursor cursor = db.query(TABLE_NAME, null, MMS_ID + " = ?", new String[] {String.valueOf(mmsId)}, null, null, null)) {
            while (cursor != null && cursor.moveToNext()) {
                results.add(new GroupReceiptInfo(SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(ADDRESS))),
