...
public class SmsMigrator {
    ...
    private static void getContentValuesForRow(Context context, Cursor cursor,
        ...
        statement.bindString(1, SignalAddress.fromExternal(context, theirAddress).serialize());
    ...
    private static @Nullable Set<SignalRecipient> getOurRecipients(Context context, String theirRecipients) {
        StringTokenizer tokenizer = new StringTokenizer(theirRecipients.trim(), " ");
        Set<SignalRecipient> recipientList = new HashSet<>();
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (address != null) {
                recipientList.add(SignalRecipient.from(context, SignalAddress.fromExternal(context, address), true));
            }
        }
...
//      return;
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                ...
                Set<SignalRecipient> ourRecipients = getOurRecipients(context, theirRecipients);
                ...
                if (ourRecipients != null) {
                    if (ourRecipients.size() == 1) {
                        ...
                        ourRecipients.add(SignalRecipient.from(context, SignalAddress.fromSerialized(TextSecurePreferences.getLocalNumber(context)), true));

                        List<SignalAddress> memberAddresses = new LinkedList<>();

                        for (SignalRecipient recipient : ourRecipients) {
                        ...
                        SignalRecipient ourGroupRecipient = SignalRecipient.from(context, SignalAddress.fromSerialized(ourGroupId), true);
