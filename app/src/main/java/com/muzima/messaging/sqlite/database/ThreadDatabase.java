...
public class ThreadDatabase extends Database {
    ...
    private long createThreadForRecipient(SignalAddress address, boolean group, int distributionType) {
    ...
    public Cursor getFilteredConversationList(@Nullable List<SignalAddress> filter) {
        if (filter == null || filter.size() == 0)
            return null;

        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        List<List<SignalAddress>> partitionedAddresses = Util.partition(filter, 900);
        List<Cursor> cursors = new LinkedList<>();

        for (List<SignalAddress> addresses : partitionedAddresses) {
    ...
    public long getThreadIdIfExistsFor(SignalRecipient recipient) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
    ...
    public long getThreadIdFor(SignalRecipient recipient) {
        return getThreadIdFor(recipient, DistributionTypes.DEFAULT);
    }

    public long getThreadIdFor(SignalRecipient recipient, int distributionType) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        String where = ADDRESS + " = ?";
        String[] recipientsArg = new String[]{recipient.getAddress().serialize()};
        Cursor cursor = null;
    ...
    public @Nullable
    SignalRecipient getRecipientForThreadId(long threadId) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        Cursor cursor = null;
    ...
    private @NonNull
    String getFormattedBodyFor(@NonNull MessageRecord messageRecord) {
    ...
    private @NonNull
    String createQuery(@NonNull String where, int limit) {
    ...
    public class Reader implements Closeable {
        ...
        public ThreadRecord getCurrent() {
            ...
            SignalAddress address = SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(ThreadDatabase.ADDRESS)));
            ...
            SignalRecipient recipient = SignalRecipient.from(context, address, settings, groupRecord, true);
