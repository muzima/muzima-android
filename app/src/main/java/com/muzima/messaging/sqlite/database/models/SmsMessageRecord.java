...
public class SmsMessageRecord extends MessageRecord {
    ...
    public SpannableString getDisplayBody() {
        if (SmsDatabase.Types.isFailedDecryptType(type)) {
            ...
            return emphasisAdded(context.getString(R.string.sms_message_record_received_corrupted_key_exchange_message));
        } else if (isInvalidVersionKeyExchange()) {
            return emphasisAdded(context.getString(R.string.sms_message_record_received_key_exchange_message_for_invalid_protocol_version));
            ...
            return emphasisAdded(context.getString(R.string.sms_message_record_received_message_with_new_safety_number_tap_to_process));
        } else if (isKeyExchange() && isOutgoing()) {
            return new SpannableString("");
        } else if (isKeyExchange() && !isOutgoing()) {
            return emphasisAdded(context.getString(R.string.conversation_item_received_key_exchange_message_tap_to_process));
            ...
            return emphasisAdded(context.getString(R.string.sms_message_record_secure_session_reset));
        } else if (isEndSession()) {
            return emphasisAdded(context.getString(R.string.sms_message_record_secure_session_reset_s, getIndividualRecipient().toShortString()));
        } else {
