package com.muzima.messaging.sqlite.database.helpers;
...
import com.muzima.messaging.crypto.AsymmetricMasterCipher;
import com.muzima.messaging.crypto.AttachmentSecretProvider;
import com.muzima.messaging.crypto.MasterCipher;
    ...
    public static void migrateCiphertext(@NonNull Context context,
        ...
        try {
            ...
            copyTable("drafts", legacyDb, modernDb, (row, progress) -> {
                ...
                try {
                    if (!TextUtils.isEmpty(draftType))
                        row.put("type", legacyCipher.decryptBody(draftType));
                    if (!TextUtils.isEmpty(draft))
                        row.put("value", legacyCipher.decryptBody(draft));
                } catch (InvalidMessageException e) {
                    Log.w(TAG, e);
                }
    ...
    private static Pair<Long, String> getPlaintextBody(@NonNull MasterCipher legacyCipher,
        ...
        try {
            if (!TextUtils.isEmpty(body)) {
                ...
                else if ((type & ENCRYPTION_ASYMMETRIC_BIT) != 0)
                    body = legacyAsymmetricCipher.decryptBody(body);
            }
