...
import com.muzima.messaging.attachments.MmsNotificationAttachment;
...
import com.muzima.messaging.jobs.TrimThreadJob;
...
import com.muzima.messaging.sqlite.database.documents.IdentityKeyMismatchList;
import com.muzima.messaging.sqlite.database.documents.NetworkFailure;
import com.muzima.messaging.sqlite.database.documents.NetworkFailureList;
...
import com.muzima.messaging.sqlite.database.models.NotificationMmsMessageRecord;
import com.muzima.messaging.sqlite.database.models.Quote;
import com.muzima.messaging.utils.Util;
import com.muzima.model.SignalRecipient;
import com.muzima.utils.JsonUtils;
...
import static com.muzima.messaging.contactshare.Contact.deserialize;
...
public class MmsDatabase extends MessagingDatabase {
    ...
    public MmsDatabase(Context context, SQLCipherOpenHelper databaseHelper) {
        ...
        this.jobManager = MuzimaApplication.getInstance(context).getJobManager();
    }
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                if (Types.isOutgoingMessageType(cursor.getLong(cursor.getColumnIndexOrThrow(MESSAGE_BOX)))) {
                    SignalAddress theirAddress = SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(ADDRESS)));
                    SignalAddress ourAddress = messageId.getAddress();
    ...
    private long getThreadIdFor(IncomingMediaMessage retrieved) throws RecipientFormattingException, MmsException {
        if (retrieved.getGroupId() != null) {
            SignalRecipient groupRecipients = SignalRecipient.from(context, retrieved.getGroupId(), true);
            return DatabaseFactory.getThreadDatabase(context).getThreadIdFor(groupRecipients);
        } else {
            SignalRecipient sender = SignalRecipient.from(context, retrieved.getFrom(), true);
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                if (Types.isSecureType(cursor.getLong(3))) {
                    SyncMessageId syncMessageId = new SyncMessageId(SignalAddress.fromSerialized(cursor.getString(1)), cursor.getLong(2));
    ...
    public OutgoingMediaMessage getOutgoingMessage(long messageId)
        ...
        try {
            ...
            if (cursor != null && cursor.moveToNext()) {
                ...
                SignalRecipient recipient = SignalRecipient.from(context, SignalAddress.fromSerialized(address), false);
                ...
                if (quoteId > 0 && (!TextUtils.isEmpty(quoteText) || !quoteAttachments.isEmpty())) {
                    quote = new QuoteModel(quoteId, SignalAddress.fromSerialized(quoteAuthor), quoteText, quoteMissing, quoteAttachments);
                }
    ...
    public long insertMessageOutbox(@NonNull OutgoingMediaMessage message,
        ...
        Map<SignalAddress, Long> earlyDeliveryReceipts = earlyDeliveryReceiptCache.remove(message.getSentTimeMillis());
        Map<SignalAddress, Long> earlyReadReceipts = earlyReadReceiptCache.remove(message.getSentTimeMillis());
        ...
        if (message.getRecipient().getAddress().isGroup()) {
            List<SignalRecipient> members = DatabaseFactory.getGroupDatabase(context).getGroupMembers(message.getRecipient().getAddress().toGroupString(), false);
            GroupReceiptDatabase receiptDatabase = DatabaseFactory.getGroupReceiptDatabase(context);

            receiptDatabase.insert(Stream.of(members).map(SignalRecipient::getAddress).toList(),
                    messageId, GroupReceiptDatabase.STATUS_UNDELIVERED, message.getSentTimeMillis());

            for (SignalAddress address : earlyDeliveryReceipts.keySet())
                receiptDatabase.update(address, messageId, GroupReceiptDatabase.STATUS_DELIVERED, -1);
            for (SignalAddress address : earlyReadReceipts.keySet())
    ...
    private @Nullable
    String getSerializedSharedContacts(long mmsId, @NonNull Map<Attachment, AttachmentId> insertedAttachmentIds, @NonNull List<Contact> contacts) {
    ...
    public static class MmsNotificationInfo {
        ...
        MmsNotificationInfo(@Nullable String from, String contentLocation, String transactionId, int subscriptionId) {
            this.from = from == null ? null : SignalAddress.fromSerialized(from);
    ...
    public class Reader implements Closeable {
        ...
        private NotificationMmsMessageRecord getNotificationMmsMessageRecord(Cursor cursor) {
            ...
            SignalRecipient recipient = getRecipientFor(address);
        ...
        private SignalRecipient getRecipientFor(String serialized) {
            SignalAddress address;

            if (TextUtils.isEmpty(serialized) || "insert-address-token".equals(serialized)) {
                address = SignalAddress.UNKNOWN;
            } else {
                address = SignalAddress.fromSerialized(serialized);

            }
            return SignalRecipient.from(context, address, true);
        }
        ...
        Quote getQuote(@NonNull Cursor cursor) {
            ...
            if (quoteId > 0 && !TextUtils.isEmpty(quoteAuthor)) {
                return new Quote(quoteId, SignalAddress.fromExternal(context, quoteAuthor), quoteText, quoteMissing, quoteDeck);
            } else {
                return null;
            }
        }
