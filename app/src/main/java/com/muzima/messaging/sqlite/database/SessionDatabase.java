...
public class SessionDatabase extends Database {
    ...
    public void store(@NonNull SignalAddress address, int deviceId, @NonNull SessionRecord record) {
    ...
    public @Nullable
    SessionRecord load(@NonNull SignalAddress address, int deviceId) {
    ...
    public @NonNull List<SessionRow> getAll() {
        ...
        try (Cursor cursor = database.query(TABLE_NAME, null, null, null, null, null, null)) {
            while (cursor != null && cursor.moveToNext()) {
                try {
                    results.add(new SessionRow(SignalAddress.fromSerialized(cursor.getString(cursor.getColumnIndexOrThrow(ADDRESS))),
                            cursor.getInt(cursor.getColumnIndexOrThrow(DEVICE)),
                            new SessionRecord(cursor.getBlob(cursor.getColumnIndexOrThrow(RECORD)))));
                } catch (IOException e) {
                    Log.w(TAG, e);
    ...
    public void delete(@NonNull SignalAddress address, int deviceId) {
        SQLiteDatabase database = databaseHelper.getWritableDatabase();
    ...
    public void deleteAllFor(@NonNull SignalAddress address) {
        SQLiteDatabase database = databaseHelper.getWritableDatabase();
