...
public class GroupDatabase extends Database  {
    ...
    public String getOrCreateGroupForMembers(List<SignalAddress> members, boolean mms) {
        Collections.sort(members);

        Cursor cursor = databaseHelper.getReadableDatabase().query(TABLE_NAME, new String[] {GROUP_ID},
                MEMBERS + " = ? AND " + MMS + " = ?",
                new String[] {SignalAddress.toSerializedList(members, ','), mms ? "1" : "0"},
                null, null, null);
        try {
            if (cursor != null && cursor.moveToNext()) {
    ...
    public @NonNull List<SignalRecipient> getGroupMembers(String groupId, boolean includeSelf) {
        ...
        for (SignalAddress member : members) {
            ...
            recipients.add(SignalRecipient.from(context, member, false));
        }

        return recipients;
    }

    public void create(@NonNull String groupId, @Nullable String title, @NonNull List<SignalAddress> members,
                       @Nullable SignalServiceAttachmentPointer avatar, @Nullable String relay)
    {
        ...
        contentValues.put(MEMBERS, SignalAddress.toSerializedList(members, ','));
        ...
        SignalRecipient.applyCached(SignalAddress.fromSerialized(groupId), recipient -> {
            recipient.setName(title);
            recipient.setGroupAvatarId(avatar != null ? avatar.getId() : null);
            recipient.setParticipants(Stream.of(members).map(memberAddress -> SignalRecipient.from(context, memberAddress, true)).toList());
        });
    ...
    public void updateTitle(String groupId, String title) {
        ...
        SignalRecipient recipient = SignalRecipient.from(context, SignalAddress.fromSerialized(groupId), false);
        recipient.setName(title);
    }
    ...
    public void updateAvatar(String groupId, byte[] avatar) {
        ...
        SignalRecipient.applyCached(SignalAddress.fromSerialized(groupId), recipient -> recipient.setGroupAvatarId(avatarId == 0 ? null : avatarId));
    }

    public void updateMembers(String groupId, List<SignalAddress> members) {
        Collections.sort(members);

        ContentValues contents = new ContentValues();
        contents.put(MEMBERS, SignalAddress.toSerializedList(members, ','));
    ...
    public void remove(String groupId, SignalAddress source) {
        List<SignalAddress> currentMembers = getCurrentMembers(groupId);
        currentMembers.remove(source);

        ContentValues contents = new ContentValues();
        contents.put(MEMBERS, SignalAddress.toSerializedList(currentMembers, ','));

        databaseHelper.getWritableDatabase().update(TABLE_NAME, contents, GROUP_ID + " = ?",
                new String[] {groupId});
    }

    private List<SignalAddress> getCurrentMembers(String groupId) {
        Cursor cursor = null;
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                return SignalAddress.fromSerializedList(serializedMembers, ',');
            }
    ...
    public static class GroupRecord {
        ...
        public List<SignalAddress> getMembers() {
            return members;
        }
