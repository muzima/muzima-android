...
import com.muzima.R;
import com.muzima.messaging.contactshare.Contact.Phone;
import com.muzima.messaging.contactshare.Contact.Email;
...
import com.muzima.utils.SpanUtil;
...
public class ContactUtil {
    ...
    private static @Nullable
        Phone getPrimaryNumber(@NonNull Contact contact) {
    ...
    private static @NonNull String getPrettyPhoneNumber(@NonNull String phoneNumber, @NonNull Locale fallbackLocale) {
        ...
        try {
            Phonenumber.PhoneNumber parsed = util.parse(phoneNumber, fallbackLocale.getISO3Country());
    ...
    @MainThread
    public static void selectRecipientThroughDialog(@NonNull Context context, @NonNull List<SignalRecipient> choices, @NonNull Locale locale, @NonNull RecipientSelectedCallback callback) {
        if (choices.size() > 1) {
    ...
    public static List<SignalRecipient> getRecipients(@NonNull Context context, @NonNull Contact contact) {
        return Stream.of(contact.getPhoneNumbers()).map(phone -> SignalRecipient.from(context, SignalAddress.fromExternal(context, phone.getNumber()), true)).toList();
    }

    @WorkerThread
    public static @NonNull
    Intent buildAddToContactsIntent(@NonNull Context context, @NonNull Contact contact) {
    ...
    public interface RecipientSelectedCallback {
        void onSelected(@NonNull SignalRecipient recipient);
    }
}
