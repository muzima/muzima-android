...
public class TextSecureIdentityKeyStore implements IdentityKeyStore {
    ...
    public boolean saveIdentity(SignalProtocolAddress address, IdentityKey identityKey, boolean nonBlockingApproval) {
        synchronized (LOCK) {
            ...
            if (!identityRecord.get().getIdentityKey().equals(identityKey)) {
                ...
                IdentityUtil.markIdentityUpdate(context, SignalRecipient.from(context, signalAddress, true));
    ...
    public IdentityKey getIdentity(SignalProtocolAddress address) {
        Optional<IdentityRecord> record = DatabaseFactory.getIdentityDatabase(context).getIdentity(SignalAddress.fromSerialized(address.getName()));
