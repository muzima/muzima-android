...
public class NotificationActivity extends BroadcastListenerActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_load:
                ...
                if (syncInProgress) {
                    Toast.makeText(this, "Already fetching notifications, ignored the request", Toast.LENGTH_SHORT).show();
                    return true;
                }

                syncNotificationsInBackgroundService();
                return true;
            default:
    ...
    protected void onReceive(Context context, Intent intent) {
        ...
        if (syncType == SYNC_NOTIFICATIONS) {
            ...
            if (syncStatus == SUCCESS) {
                notificationPagerAdapter.onNotificationDownloadFinish();
            }
        } else  {
            hideProgressbar();
        }
    ...
    private void initPager() {
        ...
        notificationPagerAdapter = new NotificationPagerAdapter(getApplicationContext(), getSupportFragmentManager(), null);
    ...
    private void onPageChange(int position) {
        ...
        switch (position) {
            case NotificationPagerAdapter.TAB_CONSULTATION:
                menubarLoadButton.setVisible(true);
                break;
        }
    }

    private void syncNotificationsInBackgroundService() {
        syncInProgress = true;
        showProgressBar();
        new SyncPatientNotificationsIntent(this).start();
    }
}
